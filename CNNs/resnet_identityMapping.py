...
class ResNet_feature_extraction_net(nn.Module):
    ...
    def __init__(self, block, layers, planes, feature_len=256):
        ...
        self.feature = nn.Linear(planes[3]*7*7*block.expansion, feature_len)
...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Linear(2048*4*4, 102)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, planes, feature_len=256, num_classes=74974):
    ...
    def forward(self, x, label=None, testing=False, extract_feature = False):
        x,latent = self.feature_extraction_net(x)
        landmark = self.discriminator(latent)
